...
pub fn app_routes(pool: web::Data<PgPool>) -> Scope {
    web::scope("/api")
        .app_data(pool.clone())
        .route("/cult_tokens/{offset}/{limit}", web::get().to(get_cult_tokens))
        .route("/top_coins", web::get().to(get_top_coins))
        .route("/cult_token/{token_address}", web::get().to(get_token_data))
        .route("/top_holders/{token_address}/{offset}/{limit}", web::get().to(get_top_holders))
        .route("/trades/{token_address}/{offset}/{limit}", web::get().to(get_token_trades))
